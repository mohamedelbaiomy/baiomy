...
class BaiomyAvatarGlow extends StatefulWidget {
  /// Creates an [BaiomyAvatarGlow] widget.
  const BaiomyAvatarGlow({
    super.key,
    required this.child,
    this.glowCount = 2,
    this.glowColor = Colors.white,
    this.glowShape = .circle,
    this.glowBorderRadius,
    this.duration = const .new(seconds: 2),
    this.startDelay,
    this.animate = true,
    this.repeat = true,
  ...
  @override
  State<BaiomyAvatarGlow> createState() => _AvatarGlowState();
}

class _AvatarGlowState extends State<BaiomyAvatarGlow>
    with SingleTickerProviderStateMixin<BaiomyAvatarGlow> {
  late final AnimationController _controller;
  late final _GlowPainter _painter;
  late final Tween<double> _opacityTween = Tween<double>(begin: 0.3, end: 0);

  void _startAnimation() async {
    final Duration? startDelay = widget.startDelay;
    if (startDelay != null) {
  ...
  @override
  void didUpdateWidget(covariant BaiomyAvatarGlow oldWidget) {
...
class _GlowPainter extends ChangeNotifier implements CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final Color glowColor = glowDecoration.color!;
    final double opacity = opacityTween.evaluate(progress);

    final Paint paint = Paint()
      ..color = glowColor.withValues(alpha: opacity)
      ..style = PaintingStyle.fill;

    final double glowSize = math.min(size.width, size.height);
    final double glowRadius = glowSize / 2;

    final double currentProgress = curve.transform(progress.value);
  ...
  void _addGlowPath(Rect glowRect) {
    _glowPath.addPath(
      glowDecoration.getClipPath(glowRect, TextDirection.ltr),
      Offset.zero,
    );
  }
